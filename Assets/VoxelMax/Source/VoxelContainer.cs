...
namespace VoxelMax
{
    ...
    public class VoxelContainer : MonoBehaviour, ISerializationCallbackReceiver
    {
        ...
        */
        ...
        private bool CheckIfInCapsulatedPoint(Vector3 aPosition, int aCheckDistance, ref List<Vector3> resultList, ref HashSet<Vector3> anotCapsulated)
        {          
            ...
            foreach (Vector3 curDirection in StaticValues.sixDirectionArray)
            {
                for (int i = 1; i <= aCheckDistance; i++)
                {
                    ...
                    if (this.voxels.ContainsKey(curPos))
                        break;//we reached the "wall"
                    if (resultList.Contains(curPos))
                        continue;//prevent recursion
                                 /*   if (!this.CheckIfInCapsulatedPoint(curPos, aCheckDistance, ref resultList))
                                        return false;//one node did not reached the wall*/
                    if (anotCapsulated.Contains(curPos))
                    {
                        result = false;
                        break;
                    }
                    if (!this.CheckIfInCapsulatedPoint(curPos, aCheckDistance, ref resultList, ref anotCapsulated))
                    {
        ...
        public void FillUpObject()
        {
            ...
            int checkDistance = Mathf.CeilToInt(Vector3.Distance(this.GetMinContainerVector(), this.GetMaxContainerVector()) / 2f);
            int stepCounter = 0;
                ...
                for (int x = (int)minVector.x; x < Mathf.Round(maxVector.x); x++)
                {
                    for (int y = (int)minVector.y; y < Mathf.Round(maxVector.y); y++)
                    {
                        for (int z = (int)minVector.z; z < Mathf.Round(maxVector.z); z++)
                        {
                            ...
                            {
                                List<Vector3> alsoEncapsulated = new List<Vector3>();
                                if (this.CheckIfInCapsulatedPoint(curPos, checkDistance, ref alsoEncapsulated, ref voxelsNotEncapsulated))
                                {
                                    voxelsToFillUp.AddRange(alsoEncapsulated);
                                }
                            }
                        }
                    }
